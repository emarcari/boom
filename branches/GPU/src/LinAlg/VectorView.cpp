...
namespace BOOM{
  namespace LinAlg{
    ...
    VV::iterator VV::end(){return iterator(V+size()*stride(), V, stride()); }
    VV::const_iterator VV::begin()const{return const_iterator(V, V, stride()); }
    VV::const_iterator VV::end()const{return const_iterator(V+size()*stride(), V, stride()); }
	...
	*x /= *y;
    ...
    double VV::normalize_prob(){
      ...
      if(s==0) throw runtime_error("normalizing constant is zero in VV::normalize_logprob");
      operator/=(s);
      return s;
    }
