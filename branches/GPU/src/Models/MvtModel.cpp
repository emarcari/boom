...
//   MVT::MvtModel(uint p, mu=0.0, sig)
//     : ParamPolicy(),
//       DataPolicy(),
//       PriorPolicy(),
//       mvn(new WeightedMvnModel(p)),
//       wgt(new ScaledChisqModel)
//   {
...
//     set_nu(nu);
  ...
  void MVT::Impute(bool sample){
    ...
    for(uint i=0; i<V.size(); ++i){
      ...
      double w = sample ? rgamma(a,b) : a/b;
      d->set_weight(w);
    }
    mvn->refresh_suf();
    wgt->refresh_suf();
  }
  void MVT::impute_latent_data(){ Impute(true); }
  void MVT::Estep(){Impute(false);}
