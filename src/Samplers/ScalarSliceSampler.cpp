...
// updates the density value
  void SSS::double_lo(double x){
    double dx = x - lo_;
    lo_  = x-2*dx;
    if(!finite(lo_)) throw_exception("infinite lower limit", x);
    logplo_ = logf_(lo_);
  }
  ...
  void SSS::check_upper_limit(double x){
    ...
    if(!finite(hi_)) throw_exception("upper limit is infinite", x);
  ...
  void SSS::check_lower_limit(double x){
    ...
    if(!finite(lo_)) throw_exception("lower limit is infininte", x);
