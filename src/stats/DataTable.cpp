...
namespace BOOM{
  ...
  inline void unknown_type(){
    throw_exception<std::runtime_error>("unknown type");
  }
  //-----------------------------------------------------------------
  ...
  DataTable::DataTable(const string &fname, bool header, const string &sep){
    ...
    if(!in){
      ...
      throw_exception<std::runtime_error>(msg + fname);
    }
  ...
  (const std::vector<bool> &include, bool add_int)const{
    ...
    if(include.size()!=nvars())
      throw_exception<std::runtime_error>("wrong sized include vector in DataTable::design");
  ...
  //============================================================
//   uint DataTable::compute_variable_index(uint n)const{
//     variable_type vt = vtypes[n];
//     uint ans = 0;
//     for(uint i = 0; i<n; ++i)
//       if(vtypes[i]==vt) ++ ans;
//     return ans;
