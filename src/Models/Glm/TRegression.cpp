...
namespace BOOM{
  ...
  void TRM::impute_latent_data(){ Impute(true); }

  void TRM::EStep(){Impute(false);}

  void TRM::Impute(bool draw){
    wreg_->suf()->clear();
    wgt_->suf()->clear();
    ...
    for(uint i=0; i<regdat.size(); ++i){
      ...
      double w = draw ? df2/ss2 : rgamma(df2, ss2);
      dp->set_weight(w);
      wreg_->suf()->update(dp);
      wgt_->suf()->update(wp);
    }
