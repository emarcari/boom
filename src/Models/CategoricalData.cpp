...
//   void CD::relabel(const StringVec &s){
//     assert(s.size()==labs_->size() && "Wrong number of labels");
//     *labs_ = s; }

//   void CD::set_order(const StringVec &s){
//     SVcI it = findstr2(lab(), s);
//     if(it==s.end()){
//       ostringstream out;
...
//   std::vector<CD> make_catdat(const StringVec &sv){
//     uint n =sv.size();
//     PK labs = unique_strings(sv);
//     std::vector<CD> ans;
//     ans.reserve(sv.size());
//     for(uint i=0; i<n; ++i){
//       CD tmp(sv[i], labs);
