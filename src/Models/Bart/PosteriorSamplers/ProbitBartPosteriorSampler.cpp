...
namespace BOOM {
  namespace Bart {
    ...
    void ProbitResidualData::add_to_probit_suf(ProbitSufficientStatistics &suf) const {
      suf.update(*this);
    }
    ...
    void ProbitSufficientStatistics::update(const ResidualRegressionData &data) {
  ...
  double ProbitBartPosteriorSampler::draw_mean(Bart::TreeNode *leaf) {
    ...
    double posterior_mean = (suf->sum() + prior_mean / prior_variance) / ivar;
