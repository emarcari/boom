...
// observations rank in the vector v.  E.g. if v[i] < v[j] then
...
class index_table_less{
 ...
 public:
  ...
  bool operator()(const uint &i, const uint &j)const{return  V[i]<V[j];}
};


template <class OBJ>
std::vector<uint> index_table(const std::vector<OBJ> &v){

  index_table_less<OBJ> Less(v);
  std::vector<uint> ans(v.size());
  for(uint i=0; i<v.size(); ++i) ans[i] = i;
...
template<class OBJ>
std::vector<uint> rank_table(const std::vector<OBJ> &v){
  std::vector<uint> indx = index_table(v);
  std::vector<uint> ans(indx.size());
  for(std::vector<uint>::size_type i = 0; i<indx.size(); ++i) ans[indx[i]] = i;
  return ans;}
