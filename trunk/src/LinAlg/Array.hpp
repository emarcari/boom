...
namespace BOOM{
  ...
  class ConstArrayBase{
   public:
    ...
    ConstArrayBase(const std::vector<int> &dims, const std::vector<int> &strides);
    ...
    // size() is the number of elements stored in the array.  It is the product of dims_;
    int size()const;
    ...
    bool operator==(const Matrix &rhs)const; // Matrix & allows for Spd comparisons as well
  ...
  class ArrayView : public ArrayBase {
   public:
    ...
    ArrayView(double *data, const std::vector<int> &dims, const std::vector<int> &strides);

    virtual       double *data(){return data_;}
    virtual const double *data()const{return data_;}

    void reset(double *data, const std::vector<int> &dims);
    void reset(double *data, const std::vector<int> &dims, const std::vector<int> &strides);
  ...
  class Array : public ArrayBase{
   public:
    ...
    // Like the slice() functions above, but exactly one index must be
    // negative.
