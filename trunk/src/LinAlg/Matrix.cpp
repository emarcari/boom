...
//       cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, nc_, nc_, nr_,
// 		  1.0, data(), nr_, data(), nr_, 0.0, ans.data(), ans.nrow());
...
//         *it *=x; }
    ...
    ostream & display(ostream & out, const Matrix &x,
		      int prec){
      out << std::setprecision(prec);
      for(uint i = 0; i<x.nrow(); ++i){
 	for(uint j = 0; j<x.ncol(); ++j)
 	  out << std::setw(8) << x(i,j) << " ";
 	out << endl;}
      return out; }
