...
namespace BOOM{
    ...
    HmmFilter::HmmFilter(std::vector<Ptr<Model> > mv, Ptr<MarkovModel> mark)
      : models_(mv),
	P(0),
	pi(mv.size()),
	logp(mv.size()),
    ...
    double HmmFilter::initialize(Ptr<Data> dp){
    ...
    double HmmFilter::fwd(const std::vector<Ptr<Data> > &dv){
      ...
      double loglike = initialize(dv[0]);
      for(uint i=1; i<n; ++i){
	for(uint s=0; s<S; ++s) logp[s] = models_[s]->pdf(dv[i], true);
        //        cout << "--------------------------------------" << endl;
        //        cout << "y[" <<i-1 <<"] = " << dv[i-1] << " y["<<i<<"]= " << dv[i] << " " << pi <<endl;
	loglike += fwd_1(pi, P[i], logQ, logp, one);
// #ifndef NDEBUG
//         if(loglike!=loglike || isnan(loglike)){
//           hmm_recursion_error(P[i], pi, logQ, logp, i, dv[i]);
//         }
// #endif
//         //        cout << P[i] << endl << pi <<endl;
      }
      return loglike;
    }
//------------------------------------------------------------
...
void HmmFilter::bkwd_sampling_mt(const std::vector<Ptr<Data> > &dv,
                                 RNG & eng){
      uint n = dv.size();
      //      pi = one * P.back();       // pi was already set by fwd.  this line breaks things when n=1;
...
//------------------------------------------------------------
	...
	s=r;
      ...
      markov_->suf()->add_initial_value(s);  // in last step of loop i
                                             // = 1, so s=h[0]
    }
//----------------------------------------------------------------------
...
//======================================================================
HmmSavePiFilter::HmmSavePiFilter(std::vector<Ptr<Model> > mv,
...
Vec HmmSavePiFilter::state_probs(Ptr<Data> dp)const{
  ...
  if(it==pi_hist_.end()){
    ostringstream err;
    err << "could not compute state probability distribution for data point "
        << *dp << endl;
...
//======================================================================
    ...
    HmmEmFilter::HmmEmFilter(std::vector<Ptr<EmMixtureComponent> > mix,
			     Ptr<MarkovModel> mark)
      : HmmFilter(std::vector<Ptr<Model> >(mix.begin(), mix.end()), mark),
	models_(mix)
    {}
//------------------------------------------------------------
