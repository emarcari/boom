...
namespace BOOM{
  ...
  NS::NeMvRegSuf(const Mat &X, const Mat &Y, bool add_int)
    : yty_(Y.ncol()),
      xtx_(X.ncol()+add_int),
      xty_(X.ncol()+add_int, Y.ncol()),
      n_(0)
  {
    QR qr;
    if(add_int) qr.decompose(cbind(1.0, X));
    else qr.decompose(X);
    Mat R = qr.getR();
    xtx_.add_inner(R);
  ...
  MvReg::MvReg(const Mat &X, const Mat &Y, bool add_int)
    : ParamPolicy(),
      DataPolicy(new QrMvRegSuf(add_int ? X : cbind(1.0, X),Y, this)),
      PriorPolicy(),
      LoglikeModel()
  {
    uint nx = X.ncol() + add_int;
    uint ny = Y.ncol();
