...
namespace BOOM{
  ...
  OCM::OrdinalCutpointModel(const Selector &Inc, uint Maxscore)
    : ParamPolicy(new GlmCoefs(Vec(Inc.nvars(), 0.0), Inc),
      ...
      PriorPolicy()
  {
    // cant call virtual function 'link_inv' in constructor
//     Vec b(beta());
//     double prob = 1.0/(1+Maxscore);
//     double b0 = -1.0*link_inv(prob);
//     b[0] = b0;
//     set_beta(b);
  }
...
//  OCM * OCM::clone()const{return new OCM(*this);}
  ...
  double OCM::log_likelihood(const Vec & beta, const Vec & delta)const{
...
//     return 0.0;
  ...
  uint OCM::size()const{return beta().size();}
